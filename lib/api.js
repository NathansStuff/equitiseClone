...
export async function getAllTestimonys() {
  const results = await client.fetch(
    `*[_type == "testimony"]{${testimonialFields}}`
  );
  return results;
}
...
export async function getAllAboutUsCard() {
  const results = await client.fetch(
    `*[_type == "aboutUsCard"]{${aboutUsFields}}`
  );
  return results;
}
...
const teamsFields = `
'teamMemberName': teamMember[0]->{name},
'teamMemberPic': teamMember[0]->{avatar},
title,
subtitle,
`;
